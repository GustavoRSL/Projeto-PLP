...
telaGravidade::telaGravidade(QWidget *parent) : QDialog(parent), ui(new Ui::telaGravidade){
	ui->setupUi(this);
	ui->variavelF->setValidator(new QDoubleValidator(0, 1000000, 2, this));
	ui->variavelG->setValidator(new QDoubleValidator(0, 1000000, 2, this));
	ui->variavelM1->setValidator(new QDoubleValidator(0, 1000000, 2, this));
	ui->variavelM2->setValidator(new QDoubleValidator(0, 1000000, 2, this));
	ui->variavelD->setValidator(new QDoubleValidator(0, 1000000, 2, this));
}
...
void telaGravidade::on_btnCalculate_clicked(){
	...
	if(vazias == 0){
		...
		if(variavel == "F"){
			double F  = (ui->variavelG->text()).toDouble();
